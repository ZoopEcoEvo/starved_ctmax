...
All statistical analyses were performed with R (Version 4.1.3; R Core Team 2022). The CT~max~ values for the
two groups (fed and starved individuals) were compared using mean difference as an effect size estimate.
Confidence intervals for this effect size were estimated using non-parametric bootstrapping (Ho et al., 2019). Two sets of effect sizes were estimated. First, we examined the difference between the initial baseline CT~max~ values and CT~max~ values for fed and starved individuals during the experiment (e.g. - the difference between fed CT~max~ and the baseline CT~max~ on each of days one through five). The second effect size was the difference between fed and starved individuals on each day within the experiment. If CT~max~ does not change over time in the fed control group, these two effect sizes will provide similar results.
